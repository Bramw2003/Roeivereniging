...
namespace Model
{
    ...
    public static class Database
    {
        ...
        public static List<Boat> GetAvailableBoats(DateTime date, DateTime start, DateTime end, Member member)
        {
            ...
            string available = @"SELECT DISTINCT [boat].[ID], [boat].[name], [types].[capacity],[types].[category],[types].[steer],[types].[sculling], (SELECT CASE WHEN EXISTS ( SELECT * FROM[brokenboat] WHERE boatID = boat.[ID] AND NOT seaworthy = 1) THEN CAST(1 AS BIT) ELSE CAST(0 AS BIT) END), removed=(SELECT CASE WHEN deleted = 0 OR deleted IS NULL THEN CAST(0 AS BIT) ELSE CAST(1 AS BIT) END),[boat].[location]
                                FROM boat 
                                ...
                                LEFT JOIN Events_boat as eb on eb.boatID = boat.ID
                                LEFT JOIN Events as e on e.ID = eb.EventsID
                                ...
                                ((@date NOT BETWEEN e.start AND e.[end]) OR (e.[start] IS NULL AND e.[end] IS NULL));";
            ...
            if (OpenConnection())
            {
                ...
                while (a.Read())
                {
                    Boat n = new Boat(id: a.GetInt32(0), name: a.GetString(1),capacity: a.GetInt32(2), category: a.GetInt32(3), steer: a.GetBoolean(4), location: a.GetString(8), sculling: a.GetBoolean(5),defect: a.GetBoolean(6),deleted: a.GetBoolean(7));
                    list.Add(n);
                }
                command.Dispose();
                connection.Close();
